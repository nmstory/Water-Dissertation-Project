...
void OGLRenderer::SetTextureRepeating(GLuint target, bool repeating) {
	...
	// glTexParameteri lets us set a variety f properties on the texture currently bound (line above) 
	// Here, coords for textures are S and T, so we set wrapping for each accordingly.
	...
	glBindTexture(GL_TEXTURE_2D, 0); // unbind texture as we're done modifying it
}
